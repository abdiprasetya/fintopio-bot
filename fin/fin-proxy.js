...
class Fintopio {
    ...
    async checkProxyIP(proxy) {
        try {
            ...
            if (response.status === 200) {
                ...
                throw new Error(`Gak bisa cek IP Proxy. Status code: ${response.status}`);
            }
        } catch (error) {
            throw new Error(`Error cek IP Proxy: ${error.message}`);
        }
    }
    ...
    async makeRequest(method, url, options = {}, proxyIndex) {
        ...
        try {
            ...
            throw new Error(`Error pake proxy ${proxy}: ${error.message}`);
        }
    }
    ...
    async auth(userData, proxyIndex) {
        ...
        try {
            ...
            this.log(`Error Authentikasi: ${error.message}`.red);
            return null;
        }
    }
    ...
    async getProfile(token, proxyIndex) {
        ...
        try {
            ...
            this.log(`Info Profile Error: ${error.message}`.red);
            return null;
        }
    }
    ...
    async checkInDaily(token, proxyIndex) {
        ...
        try {
            ...
            this.log('Sukses check in harian!'.green);
        } catch (error) {
            console.log(error);
            this.log(`Error check in!: ${error.message}`.red);
        }
    }
    ...
    async getFarmingState(token, proxyIndex) {
        ...
        try {
            ...
            this.log(`Error Status Farming: ${error.message}`.red);
            return null;
        }
    }
    ...
    async startFarming(token, proxyIndex) {
        ...
        try {
            ...
            if (finishTimestamp) {
                ...
                this.log(`Farming Selesai: ${finishTime}`.green);
            } else {
                this.log('Gak ada waktu penyelesaian'.yellow);
            }
        } catch (error) {
            this.log(`Error saat mulai farming: ${error.message}`.red);
        }
    }
    ...
    async claimFarming(token, proxyIndex) {
        ...
        try {
            ...
            this.log(`Error saat Claim Farming: ${error.message}`.red);
        }
    }
    ...
    extractFirstName(userData) {
        try {
            ...
            this.log(`Error ekstrak first_name: ${error.message}`.red);
            return 'Unknown';
        }
    }
        ...
        while (true) {
            ...
            for (let i = 0; i < users.length; i++) {
                ...
                try {
                    ...
                    this.log(`Error check proxy proxy: ${error.message}`.red);
                }
                console.log(`========== Akun ${i + 1} | ${first_name.green} | ip: ${proxyIP} ==========`);
                
                try {
                    const token = await this.auth(userData, i);
                    if (token) {
                        this.log(`Login Sukses!`.green);
                        ...
                        if (profile) {
                            ...
                            if (farmingState) {
                                if (farmingState.state === 'idling') {
                                    ...
                                    if (finishTimestamp) {
                                        const finishTime = DateTime.fromMillis(finishTimestamp).toLocaleString(DateTime.DATETIME_FULL);
                                        this.log(`Waktu Farm Selesai: ${finishTime}`.green);
                    ...
                    this.log(`Akun Proses Error ${i + 1}: ${error.message}`.red);
                    continue; 
                }
            }
            ...
            if (waitTime && waitTime > 0) {
                ...
                this.log('Timeout gk valid, segera lanjutkan pengulangan'.yellow);
